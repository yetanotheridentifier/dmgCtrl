...
- Timer (#228) — configurable countdown (default 75 min, 30–90 min in 5-min steps via Settings); starts with the game; freezes on game over; TimerDisplay in centre column; shared formatTime/TimerDisplay/TimerStepper components extracted for reuse
- Round tracker (#230)
- Help screen (#231)
...
- The settings screen (`SettingsScreen` / `SettingsScreenView`) is always available — a ⚙ button appears on the setup and game screens (top-right, alongside or below the `?` help button). The container calls both `useUserSettings()` and `useFavourites()`. The view renders five toggles (Use Hyperspace Art, Enable Epic Actions, Enable Screen Wake Lock, Enable Action Log, Enable Favourites) plus a **Force Token Display** `<select>` dropdown (Always on / LOF bases only / Always off); when **Enable Competitive Mode** is on, `TimerStepper` rows for Bo1 Timer and Bo3 Timer (both 5–90 min, step 5) and a Melee Player ID input appear below it; an **X-Wing** section (always visible) contains a `TimerStepper` for Game Timer (30–90 min, step 5, default 75); when Enable Favourites is on, a saved-bases list appears below the toggle with Remove buttons per entry and a Clear All with inline two-step confirmation. In landscape, the view renders a two-column layout (`role="group"` divs: "General settings" left, "Favourites settings" right); in portrait, a single scrollable column. `SettingsScreenView` calls `useOrientation()` directly — not via props — to avoid iOS Dynamic Type scaling issues.
