...
- `useUserSettings` hook persists user preferences to localStorage under key `user_settings` as JSON. All five preferences (`useHyperspace`, `enableForceToken`, `enableEpicActions`, `enableWakeLock`, `enableFavourites`) default to `true`. The hook handles corrupt/missing storage gracefully by falling back to defaults for any missing key.
