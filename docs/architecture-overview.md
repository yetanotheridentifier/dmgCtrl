...
- **Competitive mode** (beta, off by default) — adds a Match selector (Casual / Best of 1 / Best of 3) to the setup screen on the same row as the Format selector; enabled via the Settings screen; play mode is persisted under `pref_play_mode` and passed to the game screen; a **score panel** in the left column of the game screen tracks wins per side (Bo1: 1 marker, Bo3: 2 markers) with "Opp" at the top and "You" at the bottom; hidden in Casual mode
