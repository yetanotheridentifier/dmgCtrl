...
├── XwingSetupScreen      (standalone screen — X-Wing game setup; portrait + landscape layouts; Match dropdown (Casual/Tournament) with inline Rounds stepper (2–10) when Tournament; Ruleset dropdown (XWA enabled; Legacy/AMG/2.0/1.0 disabled) with ForwardIcon start button; List Import dropdown (None enabled; YASB/Text disabled); Your Deficit + Opp's Deficit steppers (0–4 each) shown when Import = None; settings persisted to `xwing_setup` localStorage key except deficits which are session-only)
│   ├── useXwingSetup     (hook — ruleset, matchType, rounds, listImport persisted to localStorage; playerDeficit/opponentDeficit session-only, clamped 0–4)
