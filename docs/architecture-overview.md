...
- **Round tracker** — a button in the bottom-left corner of the game screen. When the round is 0 (start phase), tapping it starts the game (advances to Round 1, adds a non-undoable Round 1 log entry). When round ≥ 1, each tap increments the round and adds a round log entry. Only visible when the action log is enabled; hidden when the match is over.
- **User settings** — persistent preferences (hyperspace art, force token, epic actions, wake lock, action log, favourites, competitive mode) accessible via the ⚙ button on the setup and game screens
...
- **Competitive mode** (beta, off by default) — adds a Match selector (Casual / Best of 1 / Best of 3) to the setup screen on the same row as the Format selector; enabled via the Settings screen; play mode is persisted under `pref_play_mode` and passed to the game screen; a **score panel** in the left column of the game screen tracks wins per side (Bo1: 1 marker, Bo3: 2 markers) with "Opp" at the top and "You" at the bottom; hidden in Casual mode; tapping "You" or "Opp" enters a pending-confirm state (button label changes to "Confirm"; tapping elsewhere cancels); tapping Confirm records the result (score increments, damage resets, "Game N Won/Lost" log entry added); reaching 0 HP auto-triggers the loss confirm; undo on the game-result log entry restores the previous score, damage, and log state; in Bo3, the counter shows "Start Game N" between games; when the match is decided, "Match Won/Lost" is displayed and the Start button, round tracker, and confirm buttons are disabled
...
│   ├── useMatch          (hook — match scores: playerScore, opponentScore, matchOver; incrementPlayerScore, incrementOpponentScore, resetMatch)
...
| **Round tracker** | A round counter button in the bottom-left corner of the game screen. When round is 0 (start phase), tapping starts the game and advances to Round 1 (initial entry is not undoable). When round ≥ 1, each tap increments the round and adds an undoable round entry. Only visible when the action log is enabled; hidden when the match is over. |
| **GameLogEntry** | The record stored by `useGameLog` for each action. Fields: `id` (UUID), `type` (string tag for styling: `hit`, `heal`, `epic`, `force-gain`, `force-use`, `monastery`, `round`, `game-result`), `message`, `color` (left strip accent), `prevState` (GameState snapshot for undo), optional `undoable` (defaults to true; set false to suppress the Undo button), optional `prevLogEntries` (full previous log state — set on `game-result` entries to restore the entire log on undo), optional `prevMatchState` (playerScore + opponentScore snapshot — set on `game-result` entries to restore the match score on undo). |
