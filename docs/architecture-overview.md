...
Core functionality:
- **Loading screen** — splash screen (icon + "LOADING") while base data is fetched; transitions to a screen determined by the `startScreen` setting: 'gameSelect' (default) → game select screen; 'swu' → SWU setup screen; 'xwing' → X-Wing setup screen; back from either game entry point always returns to game select
...
- **Round tracker** — a button in the bottom-right corner of the SWU game screen. When the round is 0 (start phase), tapping it starts the game (advances to Round 1, adds an undoable Round 1 log entry). When round ≥ 1, each tap increments the round and adds an undoable round log entry. Only visible when the action log is enabled; hidden when the match is over. X-Wing has a separate round tracker bar (see `XwingGameScreen`). Advancing the round in X-Wing resets initiative to neutral (to reflect the per-round roll-off); SWU does not reset initiative on round advance (initiative is set once per game by a roll-off and held until the end of the game).
- **Initiative bar** — a vertical bar in the left column of the game screen (between the back button and the log button) that shows who has initiative. Tapping the top half sets the counter to "OPP" (opponent has initiative); tapping the bottom half sets it to "YOU" (player has initiative). Once set, initiative cannot be returned to neutral by user action — only programmatic resets (e.g. X-Wing round advance) do so. A static "INIT" label sits in the middle of the bar at all times, identifying the bar's purpose. External "OPP" and "YOU" labels at the top and bottom edges indicate the tap targets and fade out as the counter slides over them. The counter itself fades out (opacity 0) when neutral so the static INIT label shows through. State is managed by `useInitiative`; the component is `InitiativeToggle`. Gated by the `enableInitiativeBar` setting (on by default).
- **User settings** — persistent preferences (hyperspace art, Force token display, epic actions, wake lock, action log, initiative bar, favourites, competitive mode, X-Wing phase tracker) accessible via the ⚙ button on the setup and game screens
...
- **Competitive mode** (beta, off by default) — adds a Match selector (Casual / Best of 1 / Best of 3) to the setup screen on the same row as the Format selector; enabled via the Settings screen; play mode is persisted under `pref_play_mode` and passed to the game screen; a **score panel** in the right column of the game screen (fixed 16vw from top, below the ⚙ button) is styled as a vertical bar (matching the initiative bar — same `BAR_CONTAINER_STYLE` and `BAR_LABEL_STYLE` font) containing **OPP** at the top, opponent markers, a **countdown timer / DRAW**, player markers, and **YOU** at the bottom; **OPP** and **YOU** are `flex:1` transparent tap zones (no border) in their default state, styled with `BAR_LABEL_STYLE`; tapping **OPP** replaces its label with a counter-sized **LOSE** button (`max(4vw, 28px)` height, 80% width, `--color-error` accent, `rgba(--color-error-rgb, 0.18)` background); tapping **YOU** replaces its label with a counter-sized **WIN** button (`--color-success` accent, same sizing); tapping the LOSE/WIN button confirms the result; the **DRAW** slot (between markers, in the absolutely-positioned centre block) is visible before the first game and when the timer expires — rendered as plain muted text initially; tapping it once highlights it with `--color-warning` amber (border + background); tapping it again records the draw; reaching 0 HP auto-triggers LOSE (replaces OPP); when the timer expires mid-match any recorded result also closes the match; undo on the game-result log entry restores the previous score, damage, and log state; in Bo3, the counter shows "Start Game N" between games; when the match is decided, "Match Won", "Match Lost", or "Match Drawn" is displayed
...
├── XwingSetupScreen      (standalone screen — X-Wing game setup; portrait + landscape layouts; Match dropdown (Casual/Tournament) with inline Rounds stepper (2–10) when Tournament; Scenario dropdown (None + 5 named scenarios) with random dice button; per-player List Import dropdown (None enabled → manual Deficit stepper 0–4; XWA enabled → paste XWS, validated on confirm; YASB disabled; Favourites → pick from saved squads, shown only when any exist); the opponent section is revealed once you confirm your list (portrait: below; landscape: right-hand column, placeholder until then); a confirmed squad shows a read-only name field + ☆/★ star toggle to save/remove it as a favourite (inline overwrite warning on name collision) and its pilot list; EditIcon button to revise; ForwardIcon "Go to game" appears bottom-right, under the opponent list, once both are confirmed; both List Import dropdowns + match/rounds persisted to `xwing_setup` localStorage key, deficits and scenario session-only)
│   ├── useXwingSetup     (hook — ruleset, matchType, rounds, playerListImport, opponentListImport persisted to localStorage; playerDeficit/opponentDeficit + scenario session-only, deficits clamped 0–4)
│   ├── useSquadSlot      (hook — per-player import slot: text/error/confirmed/isFromFavourite/pilots/squadName; confirm/edit/confirmFromFavourite/markUnsaved; one instance per player)
│   ├── useXwingFavourites (hook — saved squads {id,name,pilots} persisted to `xwing_favourites`; add/remove/clear)
│   └── XwingSetupScreenView (view)
├── XwingGameScreen       (standalone screen — X-Wing mission point tracker; landscape-only with RotatePrompt fallback; receives playerDeficit/opponentDeficit as props from setup screen; scores start at 0 — deficit is only awarded to a player's score when all opponent ships are destroyed (bundled with the last ship's points in a single increment so undo reverts both together); dual score counters (pre-game shows 0); three-column layout: player column (left) and opponent column (right) each contain a scrollable ShipButton list when pilots are provided — DmgCtrl icon positioned inside (player icon on the right, opponent icon on the left), name positioned outward; scenario scoring buttons (0/2/4) displayed as a 2×3 grid in the centre column, pinned to bottom, when `scenarioScoringActive`; game ends at 50 points or when the End phase is confirmed at round 12/timer-expiry; result banner (Game Won / Game Lost / Draw) replaces timer at game over; countdown timer (default 75 min, configurable in Settings) runs until game over — does not stop at round 12; a round tracker bar (rounds 1–12) spans the top between the nav buttons — current round extends as a seamless tab, tapping next segment advances the round and resets initiative to neutral; phase tracker button in centre column (gated by `enableXwingPhases`) cycles Planning→System→Activation→Engagement→End — End advances the round or ends the game; initiative locked during non-Planning phases when phase tracker enabled; inc/dec buttons hidden by default when the opposing side has ship buttons or scenario scoring is active — `enableXwingAlwaysIncDec` (Settings → X-Wing → Always Show +/− Buttons, default off) overrides this and keeps both pairs always visible; action log via `useGameHistory<XwingGameSnapshot>` + `GameLogOverlay` — all entries undoable including game-end; undoing game-start resets timer; undoing game-end resumes timer from where it was stopped)
│   ├── useGameHistory    (hook — generic ordered history; each HistoryEntry<TSnapshot> stores id, type, message, color, snapshot; add/undoLast/reset)
│   ├── useXwingGame      (hook — playerScore, opponentScore, round, gameOver, gameEnded, result; endGame() for explicit game-end; reset() clears all scores to 0; restoreState for undo)
│   ├── usePhaseTracker   (hook — generic phase cycling; accepts readonly string[] phases; returns phase, phaseIndex, isLastPhase, advance(), reset(), restore(index); XWING_PHASES exported)
│   ├── useInitiative     (hook — initiative state: 'player' | 'opponent' | null; toggleOpponent, togglePlayer, reset)
│   ├── useTimer          (hook — countdown; start, stop, reset, resume)
│   ├── useWakeLock       (hook)
│   ├── GameLogOverlay    (view — shared with SwuGameScreen)
│   ├── InitiativeToggle  (view — vertical initiative bar; OPP/YOU tap zones; sliding counter; interactive prop gates tap zones)
│   └── XwingGameScreenView (view)
...
│   ├── useGameHistory    (hook — generic action-log hook; each `HistoryEntry<SwuGameSnapshot>` records id, type, message, color, and a full game snapshot for undo; shared with X-Wing)
...
│   ├── useTimer          (hook — countdown timer: remaining seconds, isRunning, isExpired; start, stop, reset, resume; start is idempotent; stop freezes remaining without reset; resume restarts counting from original start time, accounting for elapsed time during the stopped period; wall-clock based — catches up after screen-off via visibilitychange)
│   ├── useInitiative     (hook — initiative state: 'player' | 'opponent' | null; toggleOpponent, togglePlayer, reset)
│   ├── GameLogOverlay    (view — scrollable log panel shared by SWU and X-Wing; auto-scrolls to latest entry; undo button on last undoable entry; round-type entries get a blue gradient with flat bottom corners and a top separator border, applied to all round entries including the first)
│   ├── InitiativeToggle  (view — vertical initiative bar; OPP/YOU tap zones; sliding counter)
...
├── HelpScreen         (standalone screen — renders swuSetupHelp.md, swuGameHelp.md, swuTournamentHelp.md, xwingGameHelp.md, xwingSetupHelp.md, or settingsHelp.md based on source prop: 'setup', 'game', 'tournament', 'xwing', 'xwingSetup', or 'settings'; title row: back button + icon + "Help" h1)
...
| **Action log** | The scrollable game event log shown as an overlay on the game screen when `enableActionLog` is true. Rendered by `GameLogOverlay` (shared component). Managed by `useGameHistory<TSnapshot>` in both games — `useGameHistory<SwuGameSnapshot>` for SWU, `useGameHistory<XwingGameSnapshot>` for X-Wing. Each entry shows the event message and (for the last entry) an Undo button. All entries are undoable; undoing the Round 1 / game-start entry also resets the timer. Round-type entries get a blue gradient header with a top separator border. |
| **Round tracker** | In SWU: a round counter button in the bottom-right corner of the game screen. When round is 0 (start phase), tapping starts the game and advances to Round 1 (entry is undoable; undoing it also resets the timer). When round ≥ 1, each tap increments the round and adds an undoable round entry. Only visible when the action log is enabled; hidden when the match is over. In X-Wing: a continuous 12-segment bar spanning the top of the screen; see `XwingGameScreen`. |
| **HistoryEntry** | The record stored by `useGameHistory<TSnapshot>` for each action. Fields: `id` (UUID), `type` (string tag for styling: `hit`, `heal`, `epic`, `force-gain`, `force-use`, `monastery`, `round`, `game-result`), `message`, `color` (left strip accent), `snapshot` (full game state before the action — typed as `TSnapshot`), optional `undoable` (defaults to true; set false to suppress the Undo button). `GameLogOverlay` accepts `LogEntryLike[]` (the common structural subset) so both `HistoryEntry<SwuGameSnapshot>` and `HistoryEntry<XwingGameSnapshot>` can be passed without casting. |
...
| **Loading screen** | The first screen shown on app start (`SwuLoadingScreen`). Displays the app icon and "LOADING" text. Transitions to the screen determined by `startScreen` in user settings ('gameSelect' by default) as soon as `useBases()` resolves — there is no minimum display time. |
