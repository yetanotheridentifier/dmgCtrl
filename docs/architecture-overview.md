...
| **Action log** | The scrollable game event log shown as an overlay on the game screen when `enableActionLog` is true. Rendered by `GameLogOverlay` (shared component). In SWU, managed by `useGameLog`; in X-Wing, managed by `useGameHistory<XwingGameSnapshot>`. Each entry shows the event message and (for the last entry) an Undo button. All entries are undoable; undoing the Round 1 / game-start entry also resets the timer. Round-type entries get a blue gradient header with a top separator border. |
