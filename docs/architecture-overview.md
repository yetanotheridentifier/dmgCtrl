...
- **User settings** — persistent preferences (hyperspace art, Force token display, epic actions, wake lock, action log, initiative bar, favourites, competitive mode) accessible via the ⚙ button on the setup and game screens
...
├── XwingGameScreen       (standalone screen — X-Wing mission point tracker; landscape-only with RotatePrompt fallback; pre-game deficit entry (0–4 per side, stored for future scoring use) then dual score counters starting at 0; game ends at 50 points; countdown timer (default 75 min, configurable in Settings) starts with the game and is displayed in the centre column; timer freezes at game over or round 12; result banner replaces timer at game over; a round tracker bar (rounds 1–12) spans the top between the nav buttons — the current round segment extends downward as a seamless tab (`background: var(--color-bg-dark)` prevents container shadow bleed-through), tapping the next segment advances the round and resets initiative to neutral; top-left shows `<` button + `dmgCtrl` text title before game starts, title hides once game begins; useWakeLock keeps screen on; action log via `useGameHistory<XwingGameSnapshot>` + `GameLogOverlay` — all entries undoable including Round 1 and round advances; undoing Round 1 resets the timer; score entries use semantic colours matching SWU convention; initiative bar via `useInitiative` + `InitiativeToggle`)
...
│   ├── useXwingGame      (hook — playerScore, opponentScore, round, gameOver, result; restoreState for undo)
│   ├── useInitiative     (hook — initiative state: 'player' | 'opponent' | null; toggleOpponent, togglePlayer, reset)
│   ├── useTimer          (hook — countdown; start, stop, reset)
│   ├── useWakeLock       (hook)
