...
src/
  ...
  components/
    ...
    xwingGameScreen.tsx     X-Wing game screen container — manages playerDeficit/opponentDeficit state (0–4, clamped; stored for future scoring use), gameStarted flag, analytics (onXwingGameStarted on start, onXwingGameEnded on back if game was started); deficit increment/decrement callbacks accept n: number so drag scrubber multi-step values are applied; uses RotatePrompt for portrait orientation; accepts optional onGameEnd callback (stub for timer integration in ticket #228)
    xwingGameScreenView.tsx X-Wing game screen view — pre-game: deficit entry (labels "Your deficit" / "Opp's deficit", [−][+] controls, Start Game button); in-game: labels "You" / "Opp", dual score counters; three-column layout (player column | centre column | opponent column) with full-width round tracker placeholder below; per-column layout: label → 15vmin score/deficit number → flex spacer → [−][+] buttons (14vmin each); centre column: invisible label placeholder aligns score-height area with outer scores, result banner / timer (ticket #228) centred within a fixed 15vmin-high area, spacer, Start Game button pinned to bottom (pre-game only); result banner (Game Won / Game Lost / Draw) rendered in the score-height area and replaces the timer at game over — uses --color-success / --color-error / --color-text-muted; four independent useDragScrubber instances (score + deficit per side); all callbacks typed (n: number); drag indicator offsets increment right, decrement left; counters lock (disabled) when gameOver; log button shown only when enableActionLog is true (stub — no log panel yet)
