...
src/
  ...
  utils/
    ...
    displayPilots.ts        Pilot display utility — `displayPilots(pilots: XwingPilot[]): DisplayPilot[]`; returns `{ displayName, ship, points }` per pilot; takes slug before first hyphen as display name (e.g. `bobafett-armedanddangerous` → `bobafett`); numbers duplicate base names sequentially (e.g. three `academypilot` entries → `academypilot 1/2/3`); unique names are never suffixed; `ship` passes the raw XWS ship slug through unchanged; used by setup screen view (`PilotList` helper) and game screen view; proper display names deferred to #276
  ...
  test/
    ...
    displayPilots.test.ts    Unit tests for `displayPilots()` — hyphen split, full slug when no hyphen, points preserved, ship slug preserved, empty input, unique pilots not numbered, duplicate groups numbered, only the duplicate group numbered (unique siblings unaffected), hyphenated slugs sharing same base treated as duplicates
