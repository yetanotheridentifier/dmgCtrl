...
src/
  ...
  components/
    ...
    xwingSetupScreen.tsx    X-Wing setup screen container — calls `useXwingSetup()` for persisted/session settings and two `useSquadSlot()` instances (`playerSlot`, `opponentSlot`) for per-player import state; `playerListImport`/`opponentListImport` remain separate local state (initialised from hook, player changes persisted); changing the import mode calls `slot.edit()` to clear any stale error; `onPlayerConfirm` / `onOpponentConfirm` delegate to `slot.confirm(listImport)` which calls `parseXwsText` internally; editing does not reset the other player's confirmed state; `handleStart` computes deficits: XWA → `Math.max(0, 50 − pilotTotal)`, None → hook's manual deficit; calls `onStart(playerDeficit, opponentDeficit, scenario, playerSlot.pilots, opponentSlot.pilots)`; `handleScenarioRandom` picks a random entry from `XWING_NAMED_SCENARIOS`
    xwingSetupScreenView.tsx X-Wing setup screen view — CSS grid (`gridTemplateColumns: auto 1fr auto`) aligns all label/select/button rows; `labelStyle`/`selectStyle`/`submitButtonStyle` helpers; `importErrorMessage(code)` maps parse error codes to descriptive strings; `TextareaField` helper renders textarea with SWU-style connected error box; `PilotList` helper renders a confirmed XWA list as `displayName - ship | points` rows via `displayPilots()`; textarea font fixed at 16px (iOS zoom prevention), `wordBreak: break-all` for JSON wrapping. **Portrait**: full-page scrollable grid — Match+Rounds on a single inline flex row, then Scenario, then per-player wizard rows: when confirmed XWA, pilot list appears full-width below the row; when unconfirmed XWA, textarea inline; when unconfirmed None, deficit stepper inline; opponent section hidden until `playerConfirmed || opponentConfirmed`; "Go to game" (`labelStyle` label + arrow button) appears in the title bar right section below the help button when `canStart`. **Landscape**: LHS 45% grid — Match select spans cols 2–3, Rounds full-width, Scenario, confirmed rows only (disabled select + Edit + pilot list when XWA); RHS — active unconfirmed section (select + Confirm + textarea or deficit stepper); "Go to game" at top of RHS when `canStart`.
  ...
  hooks/
    ...
    useSquadSlot.ts         Per-player import slot — encapsulates the import state for one player: `text`/`setText` (textarea value), `error` (parse error code or null), `confirmed` (locked-in flag), `pilots` (parsed `XwingPilot[]`); `confirm(listImport)` calls `parseXwsText` for XWA mode and sets error on failure, otherwise sets `confirmed = true`; `edit()` resets `confirmed` and clears `error` without touching `text` or `pilots` (so the textarea is pre-filled and a previous pilot list is still accessible); two instances are used in `XwingSetupScreen` — one per player; exports `SquadSlot` interface
    useXwingSetup.ts        X-Wing setup screen state — ruleset (XwingRuleset), matchType (XwingMatchType), rounds (2–10), `playerListImport` (XwingListImport: 'None'|'YASB'|'XWA') persisted to localStorage under key `xwing_setup`; `opponentListImport` is session-only (never persisted); playerDeficit/opponentDeficit (0–4 each) and scenario (XwingScenario) are session-only, never persisted; `load()` migrates legacy `listImport` field → `playerListImport`, and stored value `'Text'` → `'None'`; defaults: ruleset='XWA', matchType='Casual', rounds=6, playerListImport='None', scenario='None'; clamped deficit setters reject values outside 0–4; exports `XWING_NAMED_SCENARIOS` (the 5 named scenario strings, excludes 'None') and `XwingScenario` / `XwingListImport` types
  ...
  test/
    ...
    useSquadSlot.test.ts     Squad slot hook tests — initial state (empty text/null error/unconfirmed/empty pilots), `setText`, `confirm('None')` (confirmed, no pilots, no error), `confirm('XWA')` success (confirmed, pilots populated, error cleared), `confirm('XWA')` failures (empty text, invalid JSON, points < 46, < 3 ships — each sets error and blocks confirm), `edit()` resets confirmed and clears error while retaining text and pilots, re-confirm after edit
...
1. `useXwingSetup()` provides `matchType`, `rounds`, `playerListImport` (persisted to localStorage), `opponentListImport` (session-only), `playerDeficit`/`opponentDeficit` (session-only, 0–4), and `scenario`
2. Two `useSquadSlot()` instances (`playerSlot`, `opponentSlot`) own the per-player import state — `text`, `error`, `confirmed`, `pilots`; `confirm(listImport)` and `edit()` are the only mutation points
3. `playerListImport`/`opponentListImport` remain separate container state (mode selector values); changing mode calls `slot.edit()` to clear stale errors
4. Opponent section hidden until `playerSlot.confirmed || opponentSlot.confirmed`; editing one slot does not affect the other
5. XWA confirm: `slot.confirm('XWA')` calls `parseXwsText` internally — parse failure sets `slot.error` and blocks confirm
6. `handleStart` computes deficits: XWA → `Math.max(0, 50 − pilotTotal)`, None → hook's manual value; calls `onStart(playerDeficit, opponentDeficit, scenario, playerSlot.pilots, opponentSlot.pilots)` → App sets state, navigates to `'xwing'`
6. "Go to game" button hidden until both players confirmed (`canStart`); portrait — below the help button in the title bar; landscape — top of RHS column at match-row height
7. Back from the game screen returns to `'xwingSetup'`; Back from setup goes to `'gameSelect'`
