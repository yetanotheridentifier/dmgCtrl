...
src/
  ...
  components/
    ...
    xwingSetupScreenView.tsx X-Wing setup screen view — CSS grid (`gridTemplateColumns: auto 1fr auto`) aligns all label/select/button rows; `labelStyle`/`selectStyle`/`submitButtonStyle` helpers; `importErrorMessage(code)` maps parse error codes to descriptive strings; `TextareaField` helper renders textarea with SWU-style connected error box (textarea drops bottom border+radius on error; `var(--color-error)` panel connects directly below); textarea font fixed at 16px (iOS zoom prevention), `wordBreak: break-all` for JSON wrapping; Confirm uses `CheckIcon`, Edit uses small text — both use `submitButtonStyle` (same square shape as dice/random). **Portrait**: full-page scrollable grid — Match+Rounds on a single inline flex row (side by side when Tournament), then Scenario, then per-player wizard rows (Your list select + Confirm/Edit, textarea or deficit stepper inline full-width); opponent section hidden until `playerConfirmed || opponentConfirmed`; "Go to game" (`labelStyle` label + arrow button, `aria-label="Start game"`) appears in the title bar right section below the help button only when `canStart`. **Landscape**: title bar + LHS 45% + RHS flex-1; LHS grid: Match select spans cols 2–3 (RHS aligns with random button), Rounds full-width, Scenario, confirmed player/opponent rows only (disabled select + Edit); unconfirmed active section moves to RHS (select + Confirm + textarea or deficit stepper filling height); "Go to game" renders at top of RHS at match-row height when `canStart`.
