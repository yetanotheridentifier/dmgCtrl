...
src/
  ...
  components/
    ...
    xwingGameScreen.tsx     X-Wing game screen container — receives `playerDeficit`, `opponentDeficit`, `scenario`, `playerPilots`, `opponentPilots` from App; `XwingGameSnapshot = { playerScore, opponentScore, playerScenarioScore, opponentScenarioScore, round, gameStarted, phaseIndex, gameEnded, playerShips, opponentShips }` — ship states included so undo fully restores roster; `playerShips`/`opponentShips: ShipEntry[]` state initialised from pilot arrays in `handleStartGame` via `initShips()`; `handleShipAdvance(side, index, skip)` — advances a ship through `alive → half → destroyed` (tap) or `alive → destroyed` (long-press, skip=true); awards `halfPoints(pts)` on damage and `remainingPoints(pts)` on destroy (or all pts when skipping); logs `"<DisplayName> — Damaged (<pts>)"` / `"<DisplayName> — Destroyed (<pts>)"`; zero-point ships transition state but produce no score change and no log entry; `handleLogUndo` restores `playerShips`/`opponentShips` from snapshot alongside score and phase; `scenarioScoringActive` and round-advance logic unchanged
    xwingGameScreenView.tsx X-Wing game screen view — three-column layout (player | centre | opponent); pre-game: score columns display deficit starting values, centre shows "Start Game"; post-game-start: score columns show combined (ship + scenario) live scores; pilot area in each outer column switches on `gameStarted && ships.length > 0`: when active shows a scrollable column of `ShipButton` entries (alive/half visible, destroyed hidden), otherwise falls back to text pilot list pre-game or flex spacer; `ShipButton` — dmgCtrl icon (PNG) + pilot name label; icon outer side, name inner (player column: icon-left, name-right; opponent column: name-left, icon-right); `half` state renders both icon and name in `--color-warning` amber via CSS filter on image; long-press (≥400ms, ≤10px move) calls `onAdvance(index, skip=true)`; tap (pointerDown+pointerUp within 400ms) calls `onAdvance(index, skip=false)`; long-press detection uses per-button `useRef<setTimeout>` with `pointerDown`/`pointerMove`/`pointerUp`/`pointerCancel` handlers; player inc/dec omitted when the **opponent** has ship buttons (player scores via opponent ship taps instead); opponent inc/dec omitted when the **player** has ship buttons — so when only one side has a list, the other side's inc/dec remains for manual scoring; centre column: `TimerDisplay` during game, result banner at game over, scenario name subtitle when `gameStarted && scenario !== 'None'`, phase button in fixed bottom slot; scenario buttons ([0][2][4] for player, [4][2][0] for opponent) absolutely positioned at bottom of screen when `scenarioScoringActive`; `SCENARIO_BTN(selected, disabled)`; `getDisabledOpponentValues` / `getDisabledPlayerValues` — pure functions, `Set<number>` of values to disable; Chance Engagement uses `VALID_CHANCE_PAIRS = [[0,0],[2,2],[4,0],[0,4]]`; round tracker and initiative bar unchanged
  ...
  test/
    ...
    xwingGameScreen.test.tsx X-Wing game screen container tests — timer display, timer start on game start, timer stop on game over (score-based), timer does NOT stop at round 12, round tracker rendering and round advance, round advance guards, analytics, wake lock, phase tracker (visibility, cycling, End advances round, End at round 12 ends game, End with timer expired ends game, round-12 undo restores to End phase, timer-expired undo restores to End phase, undo resumes timer, phase button hidden when enableXwingPhases=false, initiative always interactive when phases disabled), initiative bar (opp/you tap zones, locked during non-Planning phases, unlocked in next Planning), score starting from opponent's deficit (props), game end result banners; pilot list display (lists rendered when pilots provided, absent when not, resolved display name shown, duplicates numbered with resolved names); ship scoring (ship buttons appear after game start, inc/dec visibility: all hidden when both have lists, player inc/dec kept when only player has list, opponent inc/dec kept when only opponent has list, tap opponent ship → player score += floor(pts/2), tap player ship → opponent score += floor(pts/2), Damaged log entry on first tap, second tap destroys and awards remaining, Destroyed log entry, long-press skips alive→destroyed with full points, zero-point ship no score/no log, undo restores score+state from alive, undo restores destroyed→half)
