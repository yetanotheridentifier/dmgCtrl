...
src/
  ...
  components/
    ...
    xwingGameScreenView.tsx X-Wing game screen view — three-column layout (player | centre | opponent); pre-game: score columns display deficit starting values, centre shows "Start Game"; post-game-start: score columns show combined (ship + scenario) live scores; pilot list (`playerPilots?`/`opponentPilots?` via `displayPilots()`) renders in the flex-1 spacer between the score number and the button slot in each outer column — empty when no pilots provided, scrollable list of pilot display names (slug before first hyphen, duplicates numbered) when provided; centre column: `TimerDisplay` during game, result banner at game over, scenario name subtitle when `gameStarted && scenario !== 'None'`, phase button in fixed bottom slot; player/opponent bottom slots (fixed `14vmin`): inc/dec buttons always visible when game started, disabled when `gameOver || scenarioScoringActive`; scenario buttons ([0][2][4] for player, [4][2][0] for opponent) absolutely positioned at bottom of screen when `scenarioScoringActive`; `SCENARIO_BTN(selected, disabled)`; `getDisabledOpponentValues` / `getDisabledPlayerValues` — pure functions, `Set<number>` of values to disable; Chance Engagement uses `VALID_CHANCE_PAIRS = [[0,0],[2,2],[4,0],[0,4]]`; round tracker and initiative bar unchanged
