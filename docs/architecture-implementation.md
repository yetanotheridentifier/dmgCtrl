...
src/
  ...
  components/
    ...
    icons.tsx               Reusable SVG icon components (BackIcon, ForwardIcon, HelpIcon, CogIcon, LogIcon, ShareIcon, HomeScreenIcon, DotsIcon, ChevronDownIcon, DiceIcon, CheckIcon, EditIcon)
    ...
    xwingSetupScreen.tsx    X-Wing setup screen container — calls `useXwingSetup()`, two `useSquadSlot()` instances, and `useXwingFavourites()`; `playerListImport`/`opponentListImport` remain separate local state, initialised from the persisted preference via `sanitizeImport` (a remembered `'Favourites'` with no saved squads falls back to `'None'`); changing either mode persists it (`setup.setPlayerListImport`/`setup.setOpponentListImport`), calls `slot.edit()`, and resets `playerManualSaved`/`opponentManualSaved`; `handlePlayerLoadFavourite` / `handleOpponentLoadFavourite` call `slot.confirmFromFavourite(fav)`; `handlePlayerSave(name)` removes any existing favourite with that name then calls `addFavourite` + sets `playerManualSaved = true`; `handlePlayerUnsave()` calls `removeFavourite` + `slot.markUnsaved()` + clears `playerManualSaved`; `playerStarFilled = playerSlot.isFromFavourite || playerManualSaved` (derived, passed to view); `handleStart` computes deficits: XWA → `Math.max(0, 50 − pilotTotal)`, None → hook's manual deficit; `handleScenarioRandom` picks a random entry from `XWING_NAMED_SCENARIOS`
    xwingSetupScreenView.tsx X-Wing setup screen view — CSS grid (`gridTemplateColumns: auto 1fr auto`) aligns all label/select/button rows; `labelStyle`/`selectStyle`/`submitButtonStyle`/`starButtonStyle`/`squadNameInputStyle`/`overwriteCancelStyle` module-level style helpers; the square action buttons (`submitButtonStyle`/`starButtonStyle` — confirm/edit/dice/go-to-game/star) match the nav buttons' footprint in portrait (`5vw` / `36px` square) and keep the `8vh` / `44px` sizing in landscape, with inner padding (`0.8vh 1.5vw`) retained in both orientations so the percentage-sized icons stay a consistent size; `selectStyle`/`squadNameInputStyle` use the same per-orientation height (`5vw`/`36px` portrait, `8vh`/`44px` landscape) so the dropdowns and read-only name field line up with those buttons; `importErrorMessage(code)` maps parse error codes to descriptive strings; `TextareaField` helper renders textarea with SWU-style connected error box; `PilotList` helper renders a confirmed list as `displayName - ship | points` rows via `displayPilots()`; Edit buttons use `<EditIcon />` (pen-on-paper SVG). When confirmed and `squadName` is defined, a squad name row renders a read-only text input spanning columns 1–2 (`gridColumn: '1 / 3'`, so its left edge lines up with the list label, accent blue border, same styling as the list selectors) plus a ☆/★ star toggle (accent-coloured) in col 3 under the Edit button; pilot list renders below this row; `playerShowOverwrite`/`opponentShowOverwrite` are view-level state: clicking ☆ when a name collision exists shows a full-width overwrite warning row (`Overwrite '{name}'?` + ★ confirm + ✕ cancel); star hidden when `squadName` is undefined. `playerSelectedFavId`/`opponentSelectedFavId` track the selected favourite in the picker `<select>`. **Import mode `'Favourites'`**: the list dropdown shows `Favourites` when `favourites.length > 0`; selecting it replaces the textarea with a `<select data-testid="player-favourites-select">` listing all saved squads; Confirm button is disabled until a squad is selected, then calls `onPlayerLoadFavourite(fav)`. **Portrait**: full-page scrollable grid — Match+Rounds, Scenario, per-player wizard rows; opponent section hidden until `playerConfirmed || opponentConfirmed`; "Go to game" floats directly under the opponent list, bottom-right, when `canStart`. **Landscape**: `renderImportColumn(side)` helper renders one player's full column (active-editing or confirmed state) for either side, so player and opponent share identical markup; shared Match/Rounds/Scenario span the top; below them a two-column row (`landscapeColStyle`, 50/50); the columns scroll vertically and therefore clip overflow, so they carry `COL_INSET` (`14px`) padding to give the buttons'/selects' glow shadows room, and the header and shared-settings rows are inset by the same `COL_INSET` so the opponent column's confirm / Go-to-game buttons line up with the header help button and the scenario dice button — player on the LHS, opponent on the RHS; the opponent column shows a placeholder (`data-testid="opponent-placeholder"`, "Confirm your list to set up your opponent") until `playerConfirmed || opponentConfirmed`, then renders the opponent's import column; "Go to game" floats under the opponent column, bottom-right, when `canStart`.
    xwingGameScreen.tsx     X-Wing game screen container — receives `playerDeficit`, `opponentDeficit`, `scenario`, `playerPilots`, `opponentPilots` from App; `XwingGameSnapshot = { playerScore, opponentScore, playerScenarioScore, opponentScenarioScore, round, gameStarted, phaseIndex, gameEnded, playerShips, opponentShips }` — ship states included so undo fully restores roster; `playerShips`/`opponentShips: ShipEntry[]` state initialised from pilot arrays in `handleStartGame` via `initShips()`; `handleStartGame` calls `game.reset()` (no args) so scores start at 0 — deficits are not applied at game start; `handleShipAdvance(side, index, skip)` — advances a ship through `alive → half → destroyed` (tap) or `alive → destroyed` (long-press, skip=true); awards `halfPoints(pts)` on damage and `remainingPoints(pts)` on destroy (or all pts when skipping); after each destruction checks `allDestroyed = nextState === 'destroyed' && updated.every(s => s.state === 'destroyed')` — when true, the relevant deficit (opponent's for a player ship, player's for an opponent ship) is bundled into the same `game.incrementOpponent/Player(pts + deficit)` call so undo reverts both together; logs `"<DisplayName> — Damaged (<pts>)"` / `"<DisplayName> — Destroyed (<pts> +N deficit)"` (suffix omitted when deficit is 0); zero-point ships transition state but produce no score change and no log entry unless the deficit applies; `handleLogUndo` restores `playerShips`/`opponentShips` from snapshot alongside score and phase; `scenarioScoringActive` and round-advance logic unchanged
    xwingGameScreenView.tsx X-Wing game screen view — three-column layout (player | centre | opponent); pre-game: score columns display 0 (deficits are not shown; they are only awarded at game end when ships are destroyed), centre shows "Start Game"; post-game-start: score columns show combined (ship + scenario) live scores; pilot area in each outer column switches on `gameStarted && ships.length > 0`: when active shows a scrollable column of `ShipButton` entries (alive/half visible, destroyed hidden), otherwise falls back to text pilot list pre-game or flex spacer; `ShipButton` — dmgCtrl icon (PNG) + pilot name label; icon inner side (player column: icon-right, name-left; opponent column: icon-left, name-right); `side: 'player' | 'opponent'` prop controls `data-testid` independently from `align` so flipping the icon position does not break test IDs; duplicate pilot names are numbered using `displayPilots()` called with the full squad array; `half` state renders both icon and name in `--color-warning` amber via CSS filter on image; long-press (≥400ms, ≤10px move) calls `onAdvance(index, skip=true)`; tap (pointerDown+pointerUp within 400ms) calls `onAdvance(index, skip=false)`; long-press detection uses per-button `useRef<setTimeout>` with `pointerDown`/`pointerMove`/`pointerUp`/`pointerCancel` handlers; player inc/dec shown when `gameStarted && (enableXwingAlwaysIncDec || (!scenarioScoringActive && !(opponentShips?.length > 0)))`; opponent inc/dec shown under the symmetric condition — so by default inc/dec are hidden when the opposing side has ship buttons or scenario scoring is active; when `enableXwingAlwaysIncDec` is true both sides' inc/dec are always visible regardless of ship lists or phase; inc/dec buttons positioned vertically equidistant between the round tracker bottom and the top of the score numbers via `calc(-3.5vw + 5vmin + min(0px, 4.5vw - 16.5vmin + env(safe-area-inset-top)/4 + env(safe-area-inset-bottom)/4))`; player `+` button right-aligns with ShipButton icon RHS; opponent `−` button left-aligns with ShipButton icon LHS; centre column: `TimerDisplay` during game, result banner at game over, scenario name subtitle when `gameStarted && scenario !== 'None'`, phase button in fixed bottom slot; scenario buttons ([0][2][4] player, [4][2][0] opponent) displayed as a 2×3 grid (`gridTemplateColumns: '5vw 5vw'`, `columnGap: 'max(2vmin, calc(4ch + 1em - 10vw))'`, `fontSize: clamp(1.2rem, 8vmin, 3.5rem)` on the container so `ch`/`em` resolve relative to the timer font) in the centre column, pinned to bottom via spacer-above strategy, when `scenarioScoringActive`; outer column button slots become empty spacers (`height: 10vmin`) to reserve alignment space; `paddingBottom` of the three-column row switches: `2vw` for scenario scoring (allows grid to reach log button bottom), `8vw` for ship-based games (prevents pilot list overlapping the absolutely-positioned log button), `2vw` pre-game; `SCENARIO_BTN(selected, disabled)`; `getDisabledOpponentValues` / `getDisabledPlayerValues` — pure functions, `Set<number>` of values to disable; Chance Engagement uses `VALID_CHANCE_PAIRS = [[0,0],[2,2],[4,0],[0,4]]`; round tracker and initiative bar unchanged
    ...
    gameSelectScreen.tsx Game select screen — entry point when startScreen is 'gameSelect' (default); header: app icon (`5vw × 5vw, minWidth/minHeight: 36px` — same footprint as `NAV_BTN_STYLE`) + `dmgCtrl` h1 (`lineHeight: 0.8`) on the left, ⚙ settings button (`NAV_BTN_STYLE`) on the right; gap between icon and title is `2vw` portrait / `1vw` landscape, matching the setup screen back-button gap so the title lands at the same horizontal position on both screens; outer padding `2vw` portrait / `2vw 2vw 3vw` landscape; logo buttons sized explicitly at `min(65vmin, 300px)` width and height — `vmin` is always the device's shorter dimension regardless of orientation, so buttons are identical in size in both portrait and landscape (≈ 255px on iPhone, capped at 300px on iPad); this explicit sizing replaces the old `flex: 1 + aspectRatio` approach which was width-constrained and overflowed the available height in landscape; `flexShrink: 0` prevents squeeze; button inner padding is `64px` (fixed, not percentage) — CSS percentage padding resolves against the containing block width, which differs between orientations (the portrait column block is 255px wide while the landscape row block is 534px wide), so a percentage would produce different logo sizes; `64px` gives ≈ 127px logo display area in both orientations; inner container is just a flex row/column (no `maxWidth`) — outer game-button area is a centred flex that distributes remaining space evenly above and below; Star Wars Unlimited and Star Wars X-Wing logo buttons (both enabled, blue accent border + glow); no help button (help content is game-specific); `onSelectSwu` navigates to the SWU setup screen; `onSelectXwing` navigates to the X-Wing setup screen
    ...
    settingsScreen.tsx   Settings screen container — reads `xwingTimerMinutes`/`setXwingTimerMinutes`, `startScreen`/`setStartScreen`, `enableXwingAlwaysIncDec`/`setEnableXwingAlwaysIncDec` from useUserSettings and passes to view alongside SWU timer values
    settingsScreenView.tsx Settings screen view — toggle list; General tab includes a **Start Screen** `<select>`; X-Wing tab contains `TimerStepper` for game timer (non-uniform values list [5.5, 30, 35, …, 90], 5.5 formatted as '5:30 (test)'), an **Enable Phase Tracker** `ToggleRow` (`id="toggle-xwing-phases"`) — controls `enableXwingPhases` setting, and an **Always Show +/− Buttons** `ToggleRow` (`id="toggle-xwing-always-inc-dec"`) — controls `enableXwingAlwaysIncDec` setting; landscape sidebar tabs use `writingMode: 'vertical-rl'` + `transform: 'rotate(180deg)'`; calls `useOrientation()` directly for iOS font sizing
  ...
  hooks/
    ...
    useXwingGame.ts         X-Wing score state — playerScore, opponentScore, playerScenarioScore, opponentScenarioScore, round (starts at 1, clamps at 12), gameOver, gameEnded, result; `gameOver = (playerScore + playerScenarioScore) ≥ 50 || (opponentScore + opponentScenarioScore) ≥ 50 || gameEnded` — combined totals; `result` computed from combined totals; `addScenarioPoints(player, opponent)` increments scenario scores (no-op when gameOver); `endGame()` sets `gameEnded = true`; `reset()` resets all scores and scenario scores to 0; `restoreState({ playerScore, opponentScore, playerScenarioScore?, opponentScenarioScore?, round, gameEnded? })` — optional scenario fields default to 0 for backward compat
    useSquadSlot.ts         Per-player import slot — encapsulates the import state for one player: `text`/`setText` (textarea value), `error` (parse error code or null), `confirmed` (locked-in flag), `isFromFavourite` (true when slot was populated via `confirmFromFavourite`), `pilots` (parsed `XwingPilot[]`), `squadName` (name string or undefined); `confirm(listImport)` calls `parseXwsText` for XWA mode and sets error on failure, otherwise sets `confirmed = true`; `edit()` resets `confirmed` and clears `error` without touching `text` or `pilots` (so the textarea is pre-filled and a previous pilot list is still accessible); `confirmFromFavourite(fav)` loads a saved squad — sets `pilots`, `squadName`, `isFromFavourite = true`, `confirmed = true`; `markUnsaved()` sets `isFromFavourite = false` without clearing `confirmed`, `pilots`, or `squadName` — used when the star toggle removes a saved squad; two instances are used in `XwingSetupScreen` — one per player; exports `SquadSlot` interface
    useXwingFavourites.ts   X-Wing squad favourites — `addFavourite(name, pilots)`, `removeFavourite(id)`, `clearFavourites()`; each favourite has `{ id, name, pilots }`; persists `XwingSquadFavourite[]` to localStorage under key `xwing_favourites`; exports `XwingSquadFavourite` type
    useXwingSetup.ts        X-Wing setup screen state — ruleset (XwingRuleset), matchType (XwingMatchType), rounds (2–10), `playerListImport` and `opponentListImport` (XwingListImport: `'None'|'YASB'|'XWA'|'Favourites'`) both persisted to localStorage under key `xwing_setup` (so both dropdowns remember the last selection); playerDeficit/opponentDeficit (0–4 each) and scenario (XwingScenario) are session-only, never persisted; `load()` migrates legacy `listImport` field → `playerListImport`, and stored value `'Text'` → `'None'`; `migrateListImport` accepts `'None'|'YASB'|'XWA'|'Favourites'` (anything else → `'None'`); defaults: ruleset='XWA', matchType='Casual', rounds=6, playerListImport='None', opponentListImport='None', scenario='None'; clamped deficit setters reject values outside 0–4; exports `XWING_NAMED_SCENARIOS` (the 5 named scenario strings, excludes 'None') and `XwingScenario` / `XwingListImport` types
    ...
    useUserSettings.ts      React Context — persistent user preferences (useHyperspace, forceTokenDisplay, enableEpicActions, enableWakeLock, enableFavourites, enableLongPress, enableActionLog, enableInitiativeBar, enableCompetitiveMode, enableGameSelect, enableXwingPhases, enableXwingAlwaysIncDec, startScreen, bo1TimerMinutes, bo3TimerMinutes, xwingTimerMinutes) backed by localStorage under key `user_settings`; forceTokenDisplay is a 3-way value ('always-on' | 'lof-only' | 'always-off'), defaulting to 'lof-only'; all boolean preferences default to `true` except enableCompetitiveMode (default false) and enableXwingAlwaysIncDec (default false); enableXwingPhases defaults to true — toggles the phase tracker button on the X-Wing game screen; enableXwingAlwaysIncDec defaults to false — when true, both inc/dec button pairs are always visible on the game screen regardless of ship lists or scenario scoring state; migrates old enableForceToken boolean (false → 'always-off', true/missing → 'lof-only'); `UserSettingsProvider` wraps the app in `main.tsx`
  ...
  data/
    xwsShipNames.ts         Static lookup — `xwsShipNames: Record<string, string>`; 89 entries mapping XWS ship slug → display name (e.g. `rz1awing` → `'RZ-1 A-wing'`); generated from xwingtmg/xwing-data2; covers all seven factions; keys starting with a digit (e.g. `'104thbattalionpilot'`) are quoted string literals to satisfy TypeScript
    xwsPilotNames.ts        Static lookup — `xwsPilotNames: Record<string, string>`; 537 entries mapping pilot base XWS slug → display name (e.g. `sabinewren` → `'Sabine Wren'`, `academypilot` → `'Academy Pilot'`); base slug is the portion before the first hyphen in the full XWS slug; generated from xwingtmg/xwing-data2; first-occurrence wins when variants share a base slug
  ...
  utils/
    ...
    displayPilots.ts        Pilot display utility — `displayPilots(pilots: XwingPilot[]): DisplayPilot[]`; returns `{ displayName, ship, points }` per pilot; resolves pilot display name via `resolveXwsName(baseSlug, xwsPilotNames)` where `baseSlug = slug.split('-')[0]`; resolves ship via `resolveXwsName(ship, xwsShipNames)`; both fall back to the raw slug if not in the lookup; numbers duplicate resolved display names sequentially (e.g. three `academypilot` entries → `Academy Pilot 1/2/3`); unique names are never suffixed; used by setup screen view (`PilotList` helper) and game screen view
    resolveXwsName.ts       Name resolver — `resolveXwsName(slug: string, map: Record<string, string>): string`; returns `map[slug] ?? slug` (falls back to raw slug when not in the map)
    shipRoster.ts           Ship state types and helpers — `ShipState = 'alive' | 'half' | 'destroyed'`; `ShipEntry = { pilot: XwingPilot; state: ShipState }`; `halfPoints(pts)` = `Math.floor(pts/2)`; `remainingPoints(pts)` = `Math.ceil(pts/2)`; `initShips(pilots)` returns `ShipEntry[]` with all ships alive; used by `xwingGameScreen.tsx` for per-ship scoring
    parseXwsText.ts         XWS list parser — `parseXwsText(text: string): ParseResult`; parses XWS JSON exported by YASB; extracts `pilots: XwingPilot[]` (each with `name`, `ship`, `points` as XWS slugs) and `total` from the top-level `points` field; validation: `too-few-ships` (< 3), `too-many-ships` (> 8), `invalid-total` (< 46 or > 50), `invalid-format` (non-JSON, missing `pilots` array, missing `points`, malformed pilot entries); zero-cost ships count toward both min and max; duplicate pilot detection deferred to #277; exports `XwingPilot` and `ParseResult` types
  ...
  test/
    ...
    xwingSetupScreen.test.tsx X-Wing setup screen container tests — ruleset dropdown absent, match dropdown defaults, Rounds stepper hidden (Casual)/shown (Tournament), scenario dropdown and random button always shown, dropdown lists None + 5 named scenarios, random button calls setScenario with a named scenario; per-player import dropdowns (XWA enabled, YASB disabled); wizard flow: opponent section hidden until player confirms, confirm/edit buttons, player edit does not reset opponent confirmed, "Go to game" button hidden (not rendered) until both confirm (`canStart`), and (portrait) rendered at the foot of the screen after the control grid; (landscape) opponent column shows `opponent-placeholder` until the player confirms, then the opponent import controls; XWA textarea shown when XWA selected, parse error blocks confirm, valid XWS reveals opponent section; pilot list (`data-testid="player-pilot-list"` / `"opponent-pilot-list"`) shown after confirming XWA/Favourites, hidden when edit clicked; onStart called with (playerDeficit, opponentDeficit, scenario, playerPilots, opponentPilots) — computed deficits for XWA, manual for None; back/help/settings callbacks; favourites: Favourites dropdown option shown only when favourites exist, favourites picker `<select>` shown when Favourites mode selected, confirm from picker shows edit button and pilot list, star button shown after confirming named squad (XWA or Favourites), hidden for unnamed/None, ☆ after XWA, ★ after Favourites load, click ☆ calls addFavourite and shows ★, click ☆ with name collision shows overwrite warning, overwrite confirm calls removeFavourite+addFavourite, overwrite cancel dismisses warning, click ★ calls removeFavourite and shows ☆; list import persistence: selecting player/opponent import calls `setPlayerListImport`/`setOpponentListImport`, dropdowns initialise from the remembered preference, remembered `'Favourites'` with no saved squads falls back to `'None'`
    displayPilots.test.ts    Unit tests for `displayPilots()` — resolves pilot name via lookup (base slug key), resolves unhyphenated slug, points preserved, ship resolved via lookup, unknown slug fallback, unknown ship fallback, empty input, unique pilots not numbered, duplicate resolved names numbered, only the duplicate group numbered (unique siblings unaffected), hyphenated slugs sharing same base treated as duplicates, numbering uses resolved display name as key
    resolveXwsName.test.ts   Unit tests for `resolveXwsName()` — known slug returns display name, known ship slug, unknown slug falls back to raw slug, empty slug falls back to empty string
    useSquadSlot.test.ts     Squad slot hook tests — initial state (empty text/null error/unconfirmed/empty pilots), `setText`, `confirm('None')` (confirmed, no pilots, no error), `confirm('XWA')` success (confirmed, pilots populated, error cleared), `confirm('XWA')` failures (empty text, invalid JSON, points < 46, < 3 ships — each sets error and blocks confirm), `edit()` resets confirmed and clears error while retaining text and pilots, re-confirm after edit; `markUnsaved()` sets `isFromFavourite = false` without clearing `confirmed`, `pilots`, or `squadName`
    useXwingSetup.test.ts    X-Wing setup hook tests — defaults (XWA/Casual/6/None/None/0/0), localStorage load for playerListImport and opponentListImport (including `'Favourites'`; not deficits), migration of legacy `listImport` field and `'Text'` value, localStorage save for playerListImport and opponentListImport (not deficits), deficit clamping at 0 and 4
    xwingGameScreen.test.tsx X-Wing game screen container tests — timer display, timer start on game start, timer stop on game over (score-based), timer does NOT stop at round 12, round tracker rendering and round advance, round advance guards, analytics, wake lock, phase tracker (visibility, cycling, End advances round, End at round 12 ends game, End with timer expired ends game, round-12 undo restores to End phase, timer-expired undo restores to End phase, undo resumes timer, phase button hidden when enableXwingPhases=false, initiative always interactive when phases disabled), initiative bar (opp/you tap zones, locked during non-Planning phases, unlocked in next Planning), scores start at 0 regardless of deficit (pre-game and post-start), game end result banners; pilot list display (lists rendered when pilots provided, absent when not, resolved display name shown, duplicates numbered with resolved names); ship scoring (ship buttons appear after game start, inc/dec visibility: all hidden when both have lists, player inc/dec kept when only player has list, opponent inc/dec kept when only opponent has list, all inc/dec shown when enableXwingAlwaysIncDec=true even with both lists, tap opponent ship → player score += floor(pts/2), tap player ship → opponent score += floor(pts/2), Damaged log entry on first tap, second tap destroys and awards remaining, Destroyed log entry, long-press skips alive→destroyed with full points, zero-point ship no score/no log, undo restores score+state from alive, undo restores destroyed→half); deficit scoring (opponent deficit not added until all opponent ships destroyed, added when last ship destroyed, player deficit mirrors this symmetrically, zero deficit adds nothing); scenario scoring: inc/dec shown during End phase when enableXwingAlwaysIncDec=true alongside scenario buttons
...
| X-Wing squad favourites | `useXwingFavourites` / localStorage | Persisted under `xwing_favourites` as JSON array of `XwingSquadFavourite` (`{ id, name, pilots }`); created/removed via the setup screen star toggle and managed in Settings → X-Wing Saved Squads; not gated by a feature flag |
...
| X-Wing setup preferences (ruleset, matchType, rounds, playerListImport, opponentListImport) | `useXwingSetup` / localStorage | Persisted under `xwing_setup` as JSON; both list-import dropdowns remember the last selection; playerDeficit/opponentDeficit and scenario are session-only (not persisted); a remembered `'Favourites'` import falls back to `'None'` at container init when no saved squads exist |
...
Deficits and other setup options are configured on `XwingSetupScreen` before the game starts. `App` stores the values returned from `onStart` and passes them to `XwingGameScreen`.

1. `useXwingSetup()` provides `matchType`, `rounds`, `playerListImport` and `opponentListImport` (both persisted to localStorage — the dropdowns remember the last selection), `playerDeficit`/`opponentDeficit` (session-only, 0–4), and `scenario`; `XwingListImport` is `'None' | 'YASB' | 'XWA' | 'Favourites'`. The container's `sanitizeImport` downgrades a remembered `'Favourites'` to `'None'` at init when no saved squads exist
2. Two `useSquadSlot()` instances (`playerSlot`, `opponentSlot`) own per-player import state — `text`, `error`, `confirmed`, `isFromFavourite`, `pilots`, `squadName`; `confirm(listImport)`, `edit()`, `confirmFromFavourite(fav)`, and `markUnsaved()` are the mutation points
3. `useXwingFavourites()` provides `favourites`, `addFavourite`, `removeFavourite`; starred state derived as `playerStarFilled = playerSlot.isFromFavourite || playerManualSaved`
4. `playerListImport`/`opponentListImport` remain separate container state; changing mode calls `slot.edit()` and resets `playerManualSaved`
5. Opponent section hidden until `playerSlot.confirmed || opponentSlot.confirmed`; editing one slot does not affect the other
6. Favourites mode: Confirm calls `slot.confirmFromFavourite(fav)`; save/unsave routed through `handlePlayerSave`/`handlePlayerUnsave` (remove-then-add for overwrite safety)
7. XWA confirm: `slot.confirm('XWA')` calls `parseXwsText` internally — parse failure sets `slot.error` and blocks confirm
8. `handleStart` computes deficits: XWA → `Math.max(0, 50 − pilotTotal)`, None → hook's manual value; calls `onStart(playerDeficit, opponentDeficit, scenario, playerSlot.pilots, opponentSlot.pilots)` → App sets state, navigates to `'xwing'`
9. "Go to game" button hidden until both players confirmed (`canStart`); rendered bottom-right in both orientations, floating directly under the opponent's list/column (portrait: below the control grid; landscape: foot of the opponent/RHS column)
10. Back from the game screen returns to `'xwingSetup'`; Back from setup goes to `'gameSelect'`
...
5. Receives `playerDeficit` and `opponentDeficit` as optional props (default 0) — passed from App which received them from the setup screen; `handleStartGame` calls `game.reset()` so scores start at 0; deficits are applied in `handleShipAdvance` at the moment the last ship on a side is destroyed — bundled with the last ship's points into the same `game.incrementOpponent/Player(pts + deficit)` call so a single undo reverts both
...
- `SettingsScreen` (container) calls `useUserSettings()`, `useFavourites()`, and `useXwingFavourites()` to read and write preferences and manage the saved SWU bases and X-Wing squad lists
...
- An **X-Wing** section (always visible, not gated by any feature flag) contains a `TimerStepper` row for **Game Timer** (min=30, max=90, step=5, default=75 minutes) with a section header label; below the timer row, an **X-Wing Saved Squads** subsection (backed by `useXwingFavourites`) lists saved squad names with individual Remove buttons and a Clear All button (two-step confirm inline); "No squads saved" shown when empty
