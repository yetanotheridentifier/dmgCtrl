...
src/
  ...
  components/
    ...
    xwingSetupScreen.tsx    X-Wing setup screen container — calls `useXwingSetup()`; wizard flow managed in container local state: `playerListImport`/`opponentListImport` (initialised from hook, with changes persisted for player only), `playerConfirmed`/`opponentConfirmed`, `playerText`/`opponentText`, `playerError`/`opponentError`, `playerPilots`/`opponentPilots`; `handlePlayerConfirm` calls `parseXwsText` for XWA mode — on parse failure sets `playerError` and aborts; `handleOpponentConfirm` mirrors this; editing does not reset the other player's confirmed state; `handleStart` computes deficits: XWA → `Math.max(0, 50 − total)`, None → hook's manual deficit; calls `onStart(playerDeficit, opponentDeficit, scenario, playerPilots, opponentPilots)`; `handleScenarioRandom` picks a random entry from `XWING_NAMED_SCENARIOS`
...
2. Container local state drives the wizard: `playerConfirmed`/`opponentConfirmed`, `playerText`/`opponentText`, `playerError`/`opponentError`, `playerPilots`/`opponentPilots`
3. Opponent section hidden until `playerConfirmed || opponentConfirmed`; editing one player does not reset the other's confirmed state
4. XWA confirm: `parseXwsText` validates pasted JSON — parse failure sets `playerError`/`opponentError` and blocks confirm
5. `handleStart` computes deficits: XWA → `Math.max(0, 50 − pilotTotal)`, None → hook's manual value; calls `onStart(playerDeficit, opponentDeficit, scenario, playerPilots, opponentPilots)` → App sets state, navigates to `'xwing'`
