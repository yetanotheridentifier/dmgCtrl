...
src/
  ...
  components/
    ...
    xwingSetupScreen.tsx    X-Wing setup screen container — calls `useXwingSetup()`; passes setup state + handlers to `XwingSetupScreenView`; `handleStart` calls `onStart(playerDeficit, opponentDeficit)` which App stores and forwards to `XwingGameScreen` as props
    xwingSetupScreenView.tsx X-Wing setup screen view — portrait + landscape layouts matching SWU setup screen style; row order: Match|Rounds → Ruleset|> start → Import → Deficits; landscape: full-width title row + 55% left column with controls; `labelStyle`/`selectStyle`/`submitButtonStyle` helpers mirror SWU; ForwardIcon start button with `aria-label="Start game"`; disabled `<option>` elements for Legacy/AMG/2.0/1.0 rulesets and YASB/Text import modes
    xwingGameScreen.tsx     X-Wing game screen container — receives `playerDeficit`/`opponentDeficit` as optional props (default 0) from App (passed from setup screen); manages gameStarted flag, logOpen flag; calls `useXwingGame()`, `useGameHistory<XwingGameSnapshot>()`, `useTimer()`, `useWakeLock()`, `useInitiative()`, `usePhaseTracker(XWING_PHASES)`; `XwingGameSnapshot = { playerScore, opponentScore, round, gameStarted, phaseIndex, gameEnded }` — captures full game state before each action; `handleStartGame` calls `log.reset()`, adds "Round 1" entry, calls `game.reset(opponentDeficit, playerDeficit)` (each player starts with the opponent's deficit per rules) and `phaseTracker.reset()`; `handlePhaseAdvance`: when `isLastPhase && (round ≥ 12 || timer.isExpired)` captures snapshot, computes result message (Game Won/Lost/Draw), calls `game.endGame()`, adds `type: 'game-result'` log entry — this makes the game-end undoable and positions the undo point at the End phase; otherwise advances round or advances phase; `handleLogUndo` captures `wasGameOver = game.gameOver` before restoring, calls `timer.reset()` if restoring to pre-game state, calls `timer.resume()` if restoring from a game-over state to an in-progress state (covers both `endGame()` and score-based game-over); `timer.stop()` via useEffect on `game.gameOver` only — no longer stops at round 12; `interactive` passed to view as `!enableXwingPhases || !gameStarted || phase === 'Planning'` to lock initiative during non-Planning phases; analytics unchanged
    xwingGameScreenView.tsx X-Wing game screen view — three-column layout (player | centre | opponent); receives `playerDeficit`/`opponentDeficit` props; pre-game: score columns display the opponent's/player's deficit as the starting value, centre column shows "Start Game" text (uses `START_TEXT_STYLE`, `8vmin`, `data-testid="start-game-btn"`) where the timer normally lives; post-game-start: score columns show live scores with [−][+] buttons; each column always renders a fixed `14vmin` bottom slot for buttons/phase-button (empty pre-game) so no layout jump occurs when game starts; centre column: `TimerDisplay` during game, result banner (Game Won / Game Lost / Draw) at game over, phase button (`data-testid="phase-btn"`, borderless uppercase muted text) in fixed bottom slot when `enableXwingPhases && gameStarted && !gameOver` — tapping calls `onPhaseAdvance`; main content area uses `justify-content: flex-end` with `padding-bottom: calc(env(safe-area-inset-bottom) + 9vw)` for consistent bottom alignment; round tracker bar absolute-positioned at top, 12 segments — current segment extends as tab, border colour follows timer threshold, tapping next segment advances round; initiative bar absolute-positioned on left, renders `InitiativeToggle` with `interactive` prop (false during non-Planning phases to lock initiative)
  ...
  hooks/
    ...
    useXwingSetup.ts        X-Wing setup screen state — ruleset (XwingRuleset), matchType (XwingMatchType), rounds (2–10), listImport (XwingListImport) persisted to localStorage under key `xwing_setup`; playerDeficit/opponentDeficit (0–4 each) are session-only state, never persisted; defaults: ruleset='XWA', matchType='Casual', rounds=6, listImport='None'; clamped deficit setters reject values outside 0–4
  ...
  test/
    ...
    xwingSetupScreen.test.tsx X-Wing setup screen container tests — dropdowns present with correct defaults, XWA only enabled, Rounds stepper hidden (Casual)/shown (Tournament), deficit steppers present (Import=None)/hidden (Import=YASB), Start Game calls onStart with playerDeficit/opponentDeficit, back/help/settings callbacks
