...
src/
  ...
  components/
    ...
    xwingGameScreen.tsx     X-Wing game screen container — manages playerDeficit/opponentDeficit state (0–4, clamped), gameStarted flag, logOpen flag; calls `useXwingGame()`, `useGameHistory<XwingGameSnapshot>()`, `useTimer()`, `useWakeLock()`, `useInitiative()`, `usePhaseTracker(XWING_PHASES)`; `XwingGameSnapshot = { playerScore, opponentScore, round, gameStarted, phaseIndex, gameEnded }` — captures full game state before each action; `handleStartGame` calls `log.reset()`, adds "Round 1" entry, calls `game.reset(opponentDeficit, playerDeficit)` (each player starts with the opponent's deficit per rules) and `phaseTracker.reset()`; `handlePhaseAdvance`: when `isLastPhase && (round ≥ 12 || timer.isExpired)` captures snapshot, computes result message (Game Won/Lost/Draw), calls `game.endGame()`, adds `type: 'game-result'` log entry — this makes the game-end undoable and positions the undo point at the End phase; otherwise advances round or advances phase; `handleLogUndo` captures `wasGameOver = game.gameOver` before restoring, calls `timer.reset()` if restoring to pre-game state, calls `timer.resume()` if restoring from a game-over state to an in-progress state (covers both `endGame()` and score-based game-over); `timer.stop()` via useEffect on `game.gameOver` only — no longer stops at round 12; `interactive` passed to view as `!enableXwingPhases || !gameStarted || phase === 'Planning'` to lock initiative during non-Planning phases; analytics unchanged
    xwingGameScreenView.tsx X-Wing game screen view — three-column layout (player | centre | opponent); pre-game deficit entry ([−][+] controls, Start Game button); in-game score counters; centre column: `TimerDisplay` during game, result banner (Game Won / Game Lost / Draw) at game over, phase button (`data-testid="phase-btn"`, borderless uppercase muted text) between scores when `enableXwingPhases && gameStarted && !gameOver` — tapping calls `onPhaseAdvance`; main content area uses `justify-content: flex-end` with `padding-bottom: calc(env(safe-area-inset-bottom) + 9vw)` for consistent bottom alignment; round tracker bar absolute-positioned at top, 12 segments — current segment extends as tab, border colour follows timer threshold, tapping next segment advances round; initiative bar absolute-positioned on left, renders `InitiativeToggle` with `interactive` prop (false during non-Planning phases to lock initiative); `InitiativeToggle` bottom padding varies by `enableActionLog` to align with button row
  ...
  hooks/
    ...
    useXwingGame.ts         X-Wing score state — playerScore, opponentScore, round (starts at 1, clamps at 12), gameOver, gameEnded, result; `gameOver = playerScore ≥ 50 || opponentScore ≥ 50 || gameEnded`; `result` computed from score comparison (>, <, =) — works for both score-based and phase-triggered endings; `endGame()` sets `gameEnded = true` (used by phase tracker to trigger explicit game-end); `reset(initialPlayerScore = 0, initialOpponentScore = 0)` accepts starting scores for deficit-based initialisation and clears `gameEnded`; `restoreState({ playerScore, opponentScore, round, gameEnded? })` restores all fields including optional `gameEnded` (defaults to false — backward compatible)
  ...
  test/
    ...
    xwingGameScreen.test.tsx X-Wing game screen container tests — timer display, timer start on game start, timer stop on game over (score-based), timer does NOT stop at round 12, round tracker rendering and round advance, round advance guards, analytics, wake lock, phase tracker (visibility, cycling, End advances round, End at round 12 ends game, End with timer expired ends game, round-12 undo restores to End phase, timer-expired undo restores to End phase, undo resumes timer, phase button hidden when enableXwingPhases=false, initiative always interactive when phases disabled), initiative bar (opp/you tap zones, locked during non-Planning phases, unlocked in next Planning), score starting from opponent's deficit, game end result banners
