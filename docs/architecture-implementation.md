...
src/
  ...
  hooks/
    ...
    useXwingFavourites.ts   X-Wing squad favourites — `addFavourite(name, pilots)`, `removeFavourite(id)`, `clearFavourites()`; each favourite has `{ id, name, pilots }`; persists `XwingSquadFavourite[]` to localStorage under key `xwing_squad_favourites`; exports `XwingSquadFavourite` type
    useXwingSetup.ts        X-Wing setup screen state — ruleset (XwingRuleset), matchType (XwingMatchType), rounds (2–10), `playerListImport` (XwingListImport: `'None'|'YASB'|'XWA'|'Favourites'`) persisted to localStorage under key `xwing_setup`; `opponentListImport` is session-only (never persisted); playerDeficit/opponentDeficit (0–4 each) and scenario (XwingScenario) are session-only, never persisted; `load()` migrates legacy `listImport` field → `playerListImport`, and stored value `'Text'` → `'None'`; defaults: ruleset='XWA', matchType='Casual', rounds=6, playerListImport='None', scenario='None'; clamped deficit setters reject values outside 0–4; exports `XWING_NAMED_SCENARIOS` (the 5 named scenario strings, excludes 'None') and `XwingScenario` / `XwingListImport` types
  ...
  test/
    ...
    useXwingSetup.test.ts    X-Wing setup hook tests — defaults (XWA/Casual/6/None/0/0/0), localStorage load for playerListImport (not opponentListImport, not deficits), migration of legacy `listImport` field and `'Text'` value, localStorage save for playerListImport (not opponentListImport, not deficits), deficit clamping at 0 and 4
...
1. `useXwingSetup()` provides `matchType`, `rounds`, `playerListImport` (persisted to localStorage), `opponentListImport` (session-only), `playerDeficit`/`opponentDeficit` (session-only, 0–4), and `scenario`; `XwingListImport` is `'None' | 'YASB' | 'XWA' | 'Favourites'`
...
9. "Go to game" button hidden until both players confirmed (`canStart`); portrait — below the help button in the title bar; landscape — top of RHS column at match-row height
