...
export function unitHasKeyword(state: GameState, unit: UnitState, name: string): boolean {
  return hasKeyword(state, unit.cardId, name) || unit.upgrades.some(({ cardId }) => hasKeyword(state, cardId, name))
}

/** A unit's total keyword numeral — values stack across the card and its upgrades. */
