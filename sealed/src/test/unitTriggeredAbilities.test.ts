...
describe('Boba Fett\'s Rancor + Greef Karga (#356)', () => {
  ...
  it('Greef Karga (119): Action creates a Mandalorian token if your base was attacked this phase', () => {
    const attacked = state({ cards: E, players: { player: player({ units: [unit('a', 'ASH_119', { arena: 'ground' })], resources: ready(3) }), opponent: player({}) }, phaseEvents: { enteredPlay: { player: [], opponent: [] }, defeated: { player: [], opponent: [] }, basesAttacked: ['player'], played: { player: [], opponent: [] } } })
