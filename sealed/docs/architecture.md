...
- The join is `Type|Name|Subtitle`, because the set listing returns Normal printings only and so
  ...
  keys, every one of the 661 variants joins to exactly one Normal row, #389).
- **Two tiers.** A bundled static map (`data/bundledPrintings.ts` + `data/printingMaps/*.json`,
  ...
  `#389` was the dynamic path's failure mode: `indexFromCache` returns a `Map` as soon as ANY
  Normal card for the set is cached, even an incomplete one, so an incomplete cache silently
  prevented the (correct, complete) network fetch from ever running. The bundled tier removes that
  race for anything it covers; re-run the generator when a new set gets ability support or FFG
  ships a new print run.
