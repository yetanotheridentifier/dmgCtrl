...
- **Upgrades & tokens**: an **upgrade** attached to a unit sits **behind** the unit card, its ability text and power/health modifier protruding below; several stack. Effects also place **tokens** on a unit, drawn as small overlays like the damage token: **Shield** (blue) soaks the next instance of damage then pops, **Experience** (amber, +1/+1), and **Advantage** (gold, labelled "adv.", +1/0 on its next attack or defence). A Shield or Advantage token that gets spent is **defeated**, exactly as a card upgrade is, so cards that watch for a defeated upgrade (Zeb Orrelios, Baylan Skoll) react to it. A stat boost that isn't printed on the card, either a temporary **"this phase" boost** (e.g. from a leader ability) or a constant **aura** from another card (e.g. a leader buffing your units), shows as a white **+X/+Y token**: the power bonus in red (top-left) and the health bonus in blue (bottom-right). A "this phase" boost clears at the end of the phase (a unit only that boost was keeping alive is then defeated); an aura lasts while its source is in play. A **Hidden** or **Sentinel** unit shows a small badge (Sentinel overrides Hidden: a hidden unit that gains Sentinel becomes attackable and drops the Hidden badge). A defeated card-upgrade returns to its **owner's** discard (which may not be the unit's controller); tokens simply vanish.
...
- **Your move**: playing, resourcing and attacking are done **by clicking**: click a playable hand card to play it (a **unit** enters play; an **upgrade** highlights the units it can attach to, click one to attach), or click a **glowing unit** then a highlighted target (enemy unit or their base) to attack. When an ability lets you pick a target, including a **leader ability** (click your **leader card** to use it, then a highlighted target) or an optional in-combat effect, the valid targets highlight on the board and a **Decline** button appears for optional ones. A "look at a card", **search**, **choose-a-card**, or **look at your resources** effect (e.g. picking which upgrade to defeat, or playing an upgrade from your resources) shows the card(s) **centre-screen** for you to pick: **click the highlighted card** to choose it (cards that aren't valid picks are revealed but dimmed); token cards (Shield, Advantage, Experience, Mandalorian) show their real art, and optional effects have a **Cancel** button (a "look at a card, then decide" effect, e.g. Reanimated Night Trooper's peek at a deck's top card, instead says **Leave** (same idea, worded for leaving the card where it is). A search **always shows you the cards it looked at, even when none of them match** what it was searching for: they are about to go to the bottom of your deck, and knowing which ones is worth having. Press **Done** to send them there. This is private: your opponent doesn't see it. When an effect then lets you choose where damage lands, the valid **units and bases** highlight on the board (either base can be chosen when the card says "a base"), and you click one. The remaining standing choices (**Mulligan, Keep hand, Take the initiative, Pass**, and skip resourcing / deploy leader / use a unit's **Action** ability) are buttons in the **Action** column of your bar.
- **Undo**: an **Undo** button appears at the foot of the **Action** column once you've taken an action. It rewinds to just before your last move, **including the opponent's reply to it**, so one click takes back the whole exchange. You can keep clicking to rewind further, all the way back to the opening mulligan; the button disappears when there's nothing left to take back, and once the game is over. Replaying the same move gives the **same** response from the opponent, so undo is a way to look again at something odd rather than a way to re-roll a bad outcome. Undone moves are left out of the saved game record.
- **What am I being asked?**: when an ability needs you to choose something on the board, a **prompt floats just below the round tracker**, between the two bases, saying what's wanted ("choose a unit to deal 2 damage to"), and where the card that caused it is known, it leads with that card. This matters most when the **opponent's** card is what triggered the decision. You also get one for the steps that aren't resolved by clicking the board: the **mulligan** (which points you at the Action buttons) and **resourcing**, both during setup, counting off the two cards you must pick, and at the regroup step where it's optional, because clicking a hand card to resource looks just like clicking one to play it. It stays quiet for the obvious things, playing a card or picking an attacker, so that when it appears it means something. It floats over the board rather than pushing it around, and never blocks a click.
- **Card names**: any card named in the **log** or in a **prompt** is shown in **bold and underlined**, coloured **blue** if you control it and **amber** if the opponent does. **Hover** the name to see the full card, with no Shift needed, unlike cards on the board. They aren't links; clicking does nothing.
- **Card printings**: a card can be printed several ways (Normal, Hyperspace, foil, prestige, showcase), each with its own collector number, and your imported deck keeps whichever you own. They all play identically: the game matches your printing to the card it is, so abilities work whatever version you opened, while the board still shows **your** art. If a printing can't be identified (you're offline, say, and that set has never been cached), the card still plays but only with its **printed stats and keywords**, and a notice above the log names exactly which cards are affected.
- **Log**: a running record of every action, in a panel on the **left** of the screen.
- **Reporting a bug**: the **bug button** in the header (beside **?**) opens a short form for a title and a description. Submitting copies a full report to your clipboard and opens GitHub's new-issue page with the title already filled in and the body left empty, so all you do is **paste and submit**. The issue is filed under **your own GitHub account**, so you need to be signed in there. The report carries the whole game so far, so the problem can be replayed exactly rather than guessed at, along with the build number and whether you were on dev or prod. If the clipboard can't be reached, the form stays open with the report shown for you to copy by hand.
...
- **Keywords** apply: Sentinel (enemies **in the same arena** must attack it, so a unit able to reach across arenas is not forced by a Sentinel in the other one), Saboteur (ignores Sentinel and defeats the defender's Shields before it attacks), Raid, Grit, Overwhelm, Restore, **Ambush** (may attack the moment it enters play), **Shielded** (enters with a Shield token), **Hidden** (can't be attacked until your next turn), **Support** (lends its abilities to another attacker), and **Advantage**.
- **Upgrades** attach to a unit, yours *or* the enemy's, and modify power/health, grant keywords, or add abilities.
- **Unique** cards (star by the name) are one-per-player. Come to control a second copy of a unique **upgrade** or **unit** and you choose one of the two to defeat: pick the upgrade in the centre-screen prompt, or click the duplicate unit on the board. (Your opponent controlling their own copy is fine, as the limit is per player.)
