...
describe('SwuGameSelectScreen', () => {
  ...
  it('renders the dmgCtrl app icon', () => {
    render(<SwuGameSelectScreen onSelectSwu={vi.fn()} onHelp={vi.fn()} />)
  ...
  it('Star Wars X-Wing button is disabled', () => {
    render(<SwuGameSelectScreen onSelectSwu={vi.fn()} onHelp={vi.fn()} />)
    expect(screen.getByRole('button', { name: /star wars x-wing/i })).toBeDisabled()
  })
  ...
  it('calls onSelectSwu when Star Wars Unlimited button is clicked', async () => {
    ...
    render(<SwuGameSelectScreen onSelectSwu={onSelectSwu} onHelp={vi.fn()} />)
  ...
  it('calls onHelp when help button is clicked', async () => {
    const onHelp = vi.fn()
    const user = userEvent.setup()
    render(<SwuGameSelectScreen onSelectSwu={vi.fn()} onHelp={onHelp} />)
